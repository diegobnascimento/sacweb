  ...
  <body>
      ...
      <div class="sticky">      
            ...
            <nav class="float-l">
              ...
              <input type=image id="botao_excluir" src="img/excluir.png" style="margin-left:3px;" width="46" height="46" onclick="excluirPedido()"> 
              ...
              <input type=image id="botao_faturar" src="img/faturar.png" style="margin-left:3px;" width="46" height="46" onclick="finalizaPedido()">
            </nav>
	  </div>
      <!-- TABLE -->
        <table class="table table-action">
			...
			if ($optipo == 'G') {
				if($id_vendedor == 'todos'){
					...
					FROM tbped WHERE pedtipo = $filtro_pedidos AND pednfesit = '' AND pednfesit != 'APOIO' AND $clifor like '%$pesquisa%' 
					ORDER BY pedcod DESC  LIMIT $limit_ini, $registros";
				} else {
					$query = "SELECT pedcod, pedncli, pednfor, peddata, pedhora, pednop, pedtot, pedtipo 
					FROM tbped WHERE pedop = $id_vendedor AND pedtipo = $filtro_pedidos AND pednfesit = '' AND pednfesit != 'APOIO' AND $clifor like '%$pesquisa%' 
					ORDER BY pedcod DESC  LIMIT $limit_ini, $registros";
				}
			} else {
				$query = "SELECT pedcod, pedncli, pednfor, peddata, pedhora, pednop, pedtot, pedtipo 
				FROM tbped WHERE pedtipo = $filtro_pedidos AND pednfesit = '' AND pednfesit != 'APOIO' AND $clifor like '%$pesquisa%' AND pedop = $op 
				ORDER BY pedcod DESC LIMIT $limit_ini, $registros";
			}
					...
					if ($dispositivo == 'mobile') {
							...
							<tr>					
							  ...
							  <?php
								if ($filtro_pedidos == '1') {
								?>
									<td id="<?php echo $linhas['pedcod']?>B" onclick="selecionaPedido(this)"><div style="display:inline-block;width:30%"><b>CLIENTE:</b></div><?php echo $linhas['pedncli']?></td>
								<?php } else { ?>
									<td id="<?php echo $linhas['pedcod']?>B" onclick="selecionaPedido(this)"><div style="display:inline-block;width:30%"><b>FORNECEDOR:</b></div><?php echo $linhas['pednfor']?></td>
								<?php } ?>							  							  
							  ...
							  if ($filtro_pedidos == '1') {
							  ?>
								<td id="<?php echo $linhas['pedcod']?>F" onclick="selecionaPedido(this)"><div style="display:inline-block;width:30%"><b>VENDEDOR:</b></div><?php echo $linhas['pednop']?></td>
							  <?php } ?>
            ...
            ?>
                ...
                <tr>
					...
					if ($filtro_pedidos == '1') {
					?>
						<td id="<?php echo $linhas['pedcod']?>B" onclick="selecionaPedido(this)"><?php echo $linhas['pedncli']?></td>
					<?php } else { ?>
						<td id="<?php echo $linhas['pedcod']?>B" onclick="selecionaPedido(this)"><?php echo $linhas['pednfor']?></td>
					<?php } ?>
					...
					if ($filtro_pedidos == '1') {
					?>
						<td id="<?php echo $linhas['pedcod']?>F" onclick="selecionaPedido(this)"><?php echo $linhas['pednop']?></td>
					<?php } ?>
